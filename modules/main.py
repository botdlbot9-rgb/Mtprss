...
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "visionias" in url:
                async with ClientSession() as session:
                    async with session.get(url, headers={
                        ...
                        'Pragma': 'no-cache', 'Referer': 'http://www.visionias.in/',
                        'User-Agent': 'Mozilla/5.0 (Linux; Android 12; RMX2121) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36'
                    }) as resp:
                        text = await resp.text()
                        url = re.search(r"(https://.*?playlist.m3u8.*?)\"", text).group(1)

            # ----- ClassPlus logic (FIXED for drmUrls) -----
                ...
                if working_token.lower() == "no":
                    await m.reply_text("⚠️ ClassPlus link requires token. Send valid token next time. Skipping.")
                    continue
                ...
                try:
                    ...
                    print(f"ClassPlus response: {res}")
                    if 'drmUrls' in res and 'manifestUrl' in res['drmUrls']:
                        url = res['drmUrls']['manifestUrl']
                    elif 'url' in res:
                        url = res['url']
                    else:
                        await m.reply_text(f"❌ ClassPlus API returned no valid URL. Response: {res}")
                        continue
                except Exception as e:
                    await m.reply_text(f"❌ ClassPlus API error: {str(e)}")
                    continue

            # ----- PW logic (PhysicsWallah) FIXED -----
            elif "d1d34p8vz63oiq" in url or "sec1.pw.live" in url:
                if working_token.lower() == "no":
                    await m.reply_text("⚠️ PW link requires token. Send valid token next time. Skipping.")
                    continue
            ...
            # yt-dlp command with quiet and no-progress to avoid float conversion error
            if "youtu" in url:
            ...
            if "jw-prod" in url:
                cmd = f'yt-dlp --quiet --no-progress -o "{name}.mp4" "{url}"'
            else:
                cmd = f'yt-dlp --quiet --no-progress -f "{ytf}" "{url}" -o "{name}.mp4"'
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    try:
                        cmd_pdf = f'yt-dlp --quiet --no-progress -o "{name}.pdf" "{url}"'
                    ...
                    # Pass cmd and name to helper (old style)
